...
import { CalendarIcon, Download, Printer, Loader2, AlertCircle, AlertTriangle, Mail, TrendingUp, TrendingDown, Minus, Eye, EyeOff, Maximize2, RefreshCw } from 'lucide-react'
import { Label } from "@/components/ui/label"
import { useQuery } from "@tanstack/react-query"
import { ScrollArea } from "@/components/ui/scroll-area"
import { useSession } from "next-auth/react"
import { AlertDialog, AlertDialogCancel, AlertDialogContent, AlertDialogDescription, AlertDialogFooter, AlertDialogHeader, AlertDialogTitle } from "@/components/ui/alert-dialog"
import { Area, AreaChart, CartesianGrid, Legend, ResponsiveContainer, XAxis, YAxis, Tooltip, ReferenceLine } from "recharts"
...
// Enhanced tooltip component with better styling and animations
const CustomTooltip = ({ active, payload, label, viewMode }: any) => {
    if (!active || !payload || !payload.length) {
        return null
    }

    const formatTooltipDate = (dateValue: any) => {
        try {
    ...
    const days = Object.entries(dailyData)
        .map(([dateStr, readings], index, array): DailyPipelineData => {
            ...
            if (index > 0) {
                ...
                const prevMetricTons = prevReadings.reduce((sum, r) => sum + r.metricTons, 0)
                const diff = metricTons - prevMetricTons
...
export default function MetricTons() {
    ...
    const {
        ...
        queryKey: ["volumeMetricsData", format(selectedMonth, "yyyy-MM")],
    ...
    // Add this useEffect inside the component, after the existing useEffect hooks
    useEffect(() => {
    ...
    const monthlyChartData = useMemo(() => {
        if (!monthlyData?.days) return []
        return monthlyData.days.filter((day) => day.readings.length > 0)
    }, [monthlyData])
    ...
    const handleExport = useCallback(() => {
        ...
        if (viewMode === "day") {
            filename = `volume-metrics-daily-${format(selectedDate, "yyyy-MM-dd")}-${timestamp}.csv`
            const headers = [
                "Time",
                "Opening Reading",
                "Closing Reading",
            ...
            filename = `volume-metrics-monthly-${format(selectedMonth, "yyyy-MM")}-${timestamp}.csv`
    ...
    const handlePrint = useCallback(() => {
        if (printRef.current) {
          ...
          <html>
            <head>
              <title>Volume Metrics Report</title>
              <style>
                    ...
                    <CardHeader className="pb-3">
                        <div className="flex justify-between items-start">
                            <div>
                                ...
                                <CardDescription>
                                    Comprehensive view of daily and monthly volume metrics with trend analysis
                                </CardDescription>
                            </div>
                            {showChart && (
                                <div className="flex flex-wrap gap-2">
                                ...
                                <div className="space-y-8" ref={printRef}>
                                                            ...
                                                            <div className="flex justify-between items-center">
                                                                <div>
                                                                    <CardTitle className="text-lg flex items-center gap-2">
                                                                        Daily Metrics - {format(selectedDate, "d MMMM yyyy")}
                                                                        <Badge variant="secondary" className="text-xs">
                                                                            {dailyChartData.length} readings
                                                                        </Badge>
                                                                    </CardTitle>
                                                                    <CardDescription>
                                                                        Hourly volume metrics with real-time updates
                                                                    </CardDescription>
                                                                </div>
                                                            </div>
                                                        </CardHeader>
                                                            ...
                                                            <div className="flex justify-between items-center">
                                                                <div>
                                                                    <CardTitle className="text-lg flex items-center gap-2">
                                                                        ...
                                                                        <Badge variant="secondary" className="text-xs">
                                                                            {monthlyChartData.length} days
                                                                        </Badge>
                                                                    </CardTitle>
                                                                    <CardDescription>
                                                                        Daily aggregated metrics with trend indicators
                                                                    </CardDescription>
                                                                </div>
                                                            </div>
                                                        </CardHeader>
                                    ...
                                    <Card className="shadow-sm">
                                        <CardHeader>
                                            ...
                                            <CardDescription>
                                                Detailed breakdown of volume metrics data
                                            </CardDescription>
                                        </CardHeader>
                                        <CardContent className="p-0">
